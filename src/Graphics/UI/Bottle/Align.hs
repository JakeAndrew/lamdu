...
hoverInPlaceOf hoverOptions@(defaultOption:_) place
    ...
    where
        ...
        pickOption surrounding =
            do
                option@(hover, _) <- focusedOptions
                guard (good surrounding hover)
                return option
            ++ focusedOptions
            & head
        good surrounding hover =
            (tl ^. _1) <= surrounding ^. Widget.sLeft &&
            (tl ^. _2) <= surrounding ^. Widget.sTop &&
            (br ^. _1) <= surrounding ^. Widget.sRight &&
            (br ^. _2) <= surrounding ^. Widget.sBottom
            where
